...
dashboard_block_df = load_parquet('./data/ai_blocks_rule.parquet')
dashboard_state_df = load_parquet('./data/ai_state_rule.parquet')
...
- **state_name** : state_name
- **state_code** : state_code 
- **dashboard_works_api** : Aggregate numbers of ongoing works counts received from NIC nrega through API
- **dashboard_works_db** :  Individual ongoing works received from NIC nrega through API
- **ongoing_works_db** : **stage2_works_api** - **stage2_works_db** (checking aggregate number and indivual fetched numbers differnce. If differnce is not 0 it means we missed that many number of works to fetch from individual api) 
...
- **ongoing_works_db** : **stage2_works_api** - **stage2_works_db** (checking aggregate number and indivual fetched numbers differnce. If differnce is not 0 it means we missed that many number of works to fetch from individual api) 
- **delta_dashboard** : **dashboard_works_api** - **dashboard_works_db** 
- **delta_dashboard_ong** :  **dashboard_works_api** - **ongoing_works_db**
"""
